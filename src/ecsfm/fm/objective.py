...
def flow_matching_loss(model, x1: jax.Array, x0: jax.Array, E: jax.Array, p: jax.Array, key: jax.random.PRNGKey):
    ...
    Args:
        ...
        key: PRNG key for sampling t and dropout
    ...
    # t ~ U(0, 1)
    key, t_key, dropout_key = jax.random.split(key, 3)
    t = jax.random.uniform(t_key, shape=(batch_size, 1))
    ...
    # Split dropout keys per batch element for stochastic dropout
    dropout_keys = jax.random.split(dropout_key, batch_size)
    v_pred = jax.vmap(
        lambda ti, xi, ei, pi, ki: model(ti, xi, ei, pi, key=ki)
    )(t, x_t, E, p, dropout_keys)
